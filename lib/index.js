 ...
 * Override the widths of some characters.  This is most easily achieved with
 * a JS Map object.
 *
 ...
 *
 * @example
 * If 'a' takes up two cells on your system for some reason:
 *
 * ```js
 * const exceptions = new Map([['a'.codePointAt(0), 2]]);
...
/**
 * @typedef {object} WidthBreak When breaking a string into max-cell lengths,
 * these are the pieces. `last` will not be included until the last segment,
 * then it will be `true`.
 *
 * @prop {string} string Multiple grapheme clusters, but never starts with a
 *   combining character unless the input string started with one.
 * @prop {number} cells Number of display cells for the cluster.
 * @prop {boolean} [last] If specified and `true`, this is the last piece.
 */
...
/**
 * Compute the widths of strings in display terminal "cells" of horizontal
 * space.  Intuitionally, a plain ASCII character takes up one cell.  Certain
 * grapheme clusters may take up -1 (e.g. backspace), 0 (e.g. escape
 * sequences), 1, or 2 (e.g. emoji) cells.
 *
 * ANSI escape sequences are stripped by default.  This can be disabled by
 * setting the includeANSI option.  Escape sequences that are paired with
 * a start sequence and a stop sequence will be processed much more gracefully
 * than individual sequences (e.g. cursor movements) or overlapping pairs.
 */
export class StringWidth {
  ...
  /**
   * Get the width of a given segment in cells.
   *
  ...
  /**
   * Ignoring ANSI, get the potential break segments for the string
   *
   * @param {string} str
